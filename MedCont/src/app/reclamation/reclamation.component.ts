...
export class ReclamationComponent {
  ...
  loadReclamation(){
    this.reclamationService.getAllReclamations().subscribe(
      (response :  Reclamation[] = []) => {
        ...
        this.toastr.success('Reclamation importé avec succès', 'Confirmation');
          },
          (error) => {
        this.loading = false;
  ...
  if (this.startDate && this.endDate) {
    ...
    filteredItems = filteredItems.filter(item => {
      const itemDate = new Date(item.dateEnvoi);
...
this.reclamationService.validerReclamation(reclamation.id).subscribe(
  (response) => {

    this.toastr.success('Reclamation valider', 'Confirmation');
    setTimeout(() => {
